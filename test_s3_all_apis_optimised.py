...
def s3_client():
    ...
    s3 = boto3.client('s3', endpoint_url=endpoint, aws_access_key_id=aws_access_key,
                      aws_secret_access_key=aws_secret_key, verify=ca_bundle_path)
    return s3
...
def test_complete_multipart_upload(s3_client):
    ...
    response = s3_client.create_multipart_upload(Bucket=bucket_name, Key=key)
    upload_id = response['UploadId']
    complete_response = s3_client.complete_multipart_upload(Bucket=bucket_name, Key=key, UploadId=upload_id,
...
def test_delete_bucket_analytics_configuration(s3_client):
    ...
    s3_client.create_bucket(Bucket=bucket_name)
    s3_client.put_bucket_analytics_configuration(Bucket=bucket_name, Id=config_id,
                                                 AnalyticsConfiguration={'Id': config_id, 'StorageClassAnalysis': {}})
...
def test_delete_bucket_intelligent_tiering_configuration(bucket_name):
    ...
    """
    try:
        response = s3_client.delete_bucket_intelligent_tiering_configuration(Bucket=bucket_name, Id='example-id')
        assert response['ResponseMetadata']['HTTPStatusCode'] == 204
...
def test_delete_bucket_inventory_configuration(bucket_name):
    ...
    """
    try:
        response = s3_client.delete_bucket_inventory_configuration(Bucket=bucket_name, Id='example-inventory-id')
        assert response['ResponseMetadata']['HTTPStatusCode'] == 204
    except ClientError as e:
        assert e.response['Error']['Code'] == 'NoSuchConfiguration'
...
def test_delete_bucket_metrics_configuration(bucket_name):
    ...
    """
    try:
        response = s3_client.delete_bucket_metrics_configuration(Bucket=bucket_name, Id='example-metrics-id')
        assert response['ResponseMetadata']['HTTPStatusCode'] == 204
    except ClientError as e:
        assert e.response['Error']['Code'] == 'NoSuchConfiguration'
...
def test_delete_objects(bucket_name, object_key):
    ...
    """
    try:
        response = s3_client.delete_objects(Bucket=bucket_name, Delete={'Objects': [{'Key': object_key}]})
        assert response['ResponseMetadata']['HTTPStatusCode'] == 200
    except ClientError as e:
        pytest.fail(f'Unexpected error: {e}')
...
def test_get_bucket_analytics_configuration(bucket_name):
    ...
    """
    try:
        response = s3_client.get_bucket_analytics_configuration(Bucket=bucket_name, Id='example-analytics-id')
...
def test_get_bucket_intelligent_tiering_configuration(bucket_name):
    ...
    """
    try:
        response = s3_client.get_bucket_intelligent_tiering_configuration(Bucket=bucket_name, Id='example-tiering-id')
...
def test_get_bucket_inventory_configuration(bucket_name):
    ...
    """
    try:
        response = s3_client.get_bucket_inventory_configuration(Bucket=bucket_name, Id='example-inventory-id')
...
def test_get_bucket_lifecycle_configuration(s3_client, bucket_name):
    ...
    try:
        ...
        assert e.response['Error'][
                   'Code'] == 'NoSuchLifecycleConfiguration', 'Expected NoSuchLifecycleConfiguration error.'
...
def test_get_bucket_ownership_controls(s3_client, bucket_name):
    ...
    try:
        ...
        assert e.response['Error'][
                   'Code'] == 'OwnershipControlsNotFoundError', 'Expected OwnershipControlsNotFoundError error.'
...
def test_get_bucket_replication(s3_client, bucket_name):
    ...
    try:
        ...
        assert e.response['Error'][
                   'Code'] == 'ReplicationConfigurationNotFoundError', 'Expected ReplicationConfigurationNotFoundError error.'
...
def test_put_bucket_analytics_configuration(bucket_name):
    ...
    s3_client.put_bucket_analytics_configuration(Bucket=bucket_name, Id=config_id,
                                                 AnalyticsConfiguration={'Id': config_id, 'StorageClassAnalysis': {
                                                     'DataExport': {'OutputSchemaVersion': 'V_1', 'Destination': {
                                                         'S3BucketDestination': {'Format': 'CSV',
...
def test_put_bucket_cors(bucket_name):
    ...
    """
    cors_configuration = {'CORSRules': [{'AllowedMethods': ['GET'], 'AllowedOrigins': ['*']}]}
...
def test_put_bucket_encryption(bucket_name):
    ...
    """
    encryption_configuration = {'Rules': [{'ApplyServerSideEncryptionByDefault': {'SSEAlgorithm': 'AES256'}}]}
...
def test_put_bucket_intelligent_tiering_configuration(bucket_name):
    ...
    s3_client.put_bucket_intelligent_tiering_configuration(Bucket=bucket_name, Id=config_id,
                                                           IntelligentTieringConfiguration={'Id': config_id,
                                                                                            'Status': 'Enabled',
                                                                                            'Tierings': [{'Days': 30,
...
def test_put_bucket_inventory_configuration(bucket_name):
    ...
    s3_client.put_bucket_inventory_configuration(Bucket=bucket_name, Id=config_id, InventoryConfiguration={
        'Destination': {'S3BucketDestination': {'Bucket': f'arn:aws:s3:::{bucket_name}', 'Format': 'CSV'}},
        'IsEnabled': True, 'Id': config_id, 'IncludedObjectVersions': 'All', 'Schedule': {'Frequency': 'Daily'}})
...
def test_put_bucket_lifecycle(bucket_name):
    ...
    lifecycle_configuration = {
        'Rules': [{'ID': 'test-rule', 'Prefix': '', 'Status': 'Enabled', 'Expiration': {'Days': 365}}]}
...
def test_put_bucket_lifecycle_configuration(bucket_name):
    ...
    """
    lifecycle_configuration = {
        'Rules': [{'ID': 'test-rule', 'Filter': {'Prefix': ''}, 'Status': 'Enabled', 'Expiration': {'Days': 365}}]}
...
def test_put_bucket_logging(s3_client, bucket_name):
    ...
    """
    logging_config = {'LoggingEnabled': {'TargetBucket': bucket_name, 'TargetPrefix': 'logs/'}}
    response = s3_client.put_bucket_logging(Bucket=bucket_name, BucketLoggingStatus=logging_config)
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_put_bucket_metrics_configuration(s3_client, bucket_name):
    ...
    """
    metrics_config = {'Id': 'MetricsConfig', 'Filter': {'Prefix': 'logs/'}}
    response = s3_client.put_bucket_metrics_configuration(Bucket=bucket_name, Id='MetricsConfig',
...
def test_put_bucket_notification(s3_client, bucket_name):
    ...
    notification_config = {'TopicConfigurations': [
        {'Id': 'ExampleTopicConfiguration', 'TopicArn': 'arn:aws:sns:us-east-1:123456789012:MyTopic',
         'Events': ['s3:ObjectCreated:*']}]}
    response = s3_client.put_bucket_notification(Bucket=bucket_name, NotificationConfiguration=notification_config)
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_put_bucket_notification_configuration(s3_client, bucket_name):
    ...
    notification_config = {'TopicConfigurations': [
        {'Id': 'ExampleTopicConfiguration', 'TopicArn': 'arn:aws:sns:us-east-1:123456789012:MyTopic',
         'Events': ['s3:ObjectCreated:*']}]}
    response = s3_client.put_bucket_notification_configuration(Bucket=bucket_name,
...
def test_put_bucket_ownership_controls(s3_client, bucket_name):
    ...
    response = s3_client.put_bucket_ownership_controls(Bucket=bucket_name, OwnershipControls=ownership_controls)
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_put_bucket_replication(s3_client, bucket_name):
    ...
    """
    replication_config = {'Role': 'arn:aws:iam::123456789012:role/replication-role', 'Rules': [
        {'ID': 'ReplicationRule1', 'Status': 'Enabled', 'Prefix': '',
         'Destination': {'Bucket': 'arn:aws:s3:::destination-bucket'}}]}
    response = s3_client.put_bucket_replication(Bucket=bucket_name, ReplicationConfiguration=replication_config)
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_put_bucket_request_payment(s3_client, bucket_name):
    ...
    """
    request_payment_config = {'Payer': 'Requester'}
    response = s3_client.put_bucket_request_payment(Bucket=bucket_name,
...
def test_put_bucket_tagging(s3_client, bucket_name):
    ...
    """
    tagging_config = {'TagSet': [{'Key': 'Environment', 'Value': 'Test'}]}
    response = s3_client.put_bucket_tagging(Bucket=bucket_name, Tagging=tagging_config)
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_put_bucket_versioning(s3_client, bucket_name):
    ...
    response = s3_client.put_bucket_versioning(Bucket=bucket_name, VersioningConfiguration=versioning_config)
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_put_bucket_website():
    ...
    response = s3_client.put_bucket_website(Bucket=BUCKET_NAME, WebsiteConfiguration=website_configuration)
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_put_object():
    ...
    """
    response = s3_client.put_object(Bucket=BUCKET_NAME, Key=OBJECT_KEY, Body=b'Hello, world!')
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_put_object_acl():
    ...
    """
    response = s3_client.put_object_acl(Bucket=BUCKET_NAME, Key=OBJECT_KEY, ACL='public-read')
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_put_object_legal_hold():
    ...
    response = s3_client.put_object_legal_hold(Bucket=BUCKET_NAME, Key=OBJECT_KEY, LegalHold=legal_hold)
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_put_object_lock_configuration():
    ...
    """
    lock_configuration = {'ObjectLockEnabled': 'Enabled',
                          'Rule': {'DefaultRetention': {'Mode': 'GOVERNANCE', 'Days': 1}}}
    response = s3_client.put_object_lock_configuration(Bucket=BUCKET_NAME, ObjectLockConfiguration=lock_configuration)
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_put_object_retention():
    ...
    """
    retention = {'Mode': 'GOVERNANCE', 'RetainUntilDate': '2023-12-31T00:00:00'}
    response = s3_client.put_object_retention(Bucket=BUCKET_NAME, Key=OBJECT_KEY, Retention=retention)
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_put_object_tagging():
    ...
    """
    tagging = {'TagSet': [{'Key': 'Project', 'Value': 'Test'}]}
    response = s3_client.put_object_tagging(Bucket=BUCKET_NAME, Key=OBJECT_KEY, Tagging=tagging)
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_restore_object():
    ...
    """
    restore_request = {'Days': 1, 'GlacierJobParameters': {'Tier': 'Standard'}}
    response = s3_client.restore_object(Bucket=BUCKET_NAME, Key=OBJECT_KEY, RestoreRequest=restore_request)
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_select_object_content():
    ...
    response = s3_client.select_object_content(Bucket=BUCKET_NAME, Key=OBJECT_KEY, ExpressionType='SQL',
                                               Expression=expression, InputSerialization=input_serialization,
...
def test_upload_part():
    ...
    """
    response = s3_client.upload_part(Bucket=BUCKET_NAME, Key=OBJECT_KEY, PartNumber=PART_NUMBER, UploadId=UPLOAD_ID,
                                     Body=b'Part data')
    assert response['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_upload_part_copy(s3_client):
    ...
    part_number = 1
    try:
        response = s3_client.upload_part_copy(Bucket=destination_bucket, Key=destination_key,
...
def test_upload_part_copy_invalid_source(s3_client):
    ...
    with pytest.raises(ClientError) as excinfo:
        s3_client.upload_part_copy(Bucket=destination_bucket, Key=destination_key,
                                   CopySource={'Bucket': source_bucket, 'Key': source_key}, UploadId=upload_id,
